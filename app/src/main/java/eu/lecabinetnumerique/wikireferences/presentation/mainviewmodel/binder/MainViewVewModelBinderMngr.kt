package eu.lecabinetnumerique.wikireferences.presentation.mainviewmodel.binder
...
import eu.lecabinetnumerique.wikireferences.databinding.ActivityMainBinding
...
fun hideIfLoading(view: View, liveSearchState: LiveData<SearchStateModel>) {
    view.visibility = if (liveSearchState.value== SearchStateModel.Searching) View.GONE else View.VISIBLE
}

@BindingAdapter("showIfLoading")
fun showIfLoading(view: View, liveSearchState: LiveData<SearchStateModel>) {
    view.visibility = if (liveSearchState.value== SearchStateModel.Searching) View.VISIBLE else View.GONE
}
...
object MainViewVewModelBinderMngr {
